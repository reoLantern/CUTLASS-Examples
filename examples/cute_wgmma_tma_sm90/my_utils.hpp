...
// -------------------- 打印 [地址偏移, 实际数据] 格式  -------------------
template <class Layout, class DT> CUTE_HOST_DEVICE
void print_data_with_layout(Layout const &layout, DT const *data)
{
    CUTE_STATIC_ASSERT_V(rank(layout) == cute::Int<2>{});
    ...
    // 字符 ", " 总共需要 2 个额外字符，因此新的单元格宽度：
    int cell_width = 2 * idx_width;
    ...
    for (int m = 0; m < cute::size<0>(layout); ++m)
    {
        ...
        {
            int offset = layout(m, n);
            printf("| %*d, %*g ", idx_width - 2, offset, idx_width - 2, float(data[offset]));
        }
        printf("|\n");
    }
    // 打印表格最后一行的分割线
